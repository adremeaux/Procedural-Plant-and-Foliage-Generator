...
namespace BionicWombat {
  ...
  public class LeafShape {
    ...
    public void Heartify(LeafParamDict fields) {
      if (!fields[LPK.Heart].enabled) return;
      ...
      CurveJoin.FlattenAngle(c0, c1, deps); //account for sheer and pudge
    ...
    public void Lobes(LeafParamDict fields) {
      if (!fields[LPK.Lobes].enabled || !fields[LPK.Heart].enabled) return;
    ...
    public void FlattenAngle(int index) {
      ...
      if (index < curves.Count) {
        ...
        if (c0.nextCurve != null) {
          CurveJoin.FlattenAngle(c0, c0.nextCurve, deps);
          return;
        }
      }
