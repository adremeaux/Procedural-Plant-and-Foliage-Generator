...
namespace BionicWombat {
  ...
  public class LeafVeins {
    ...
    public void Render(LeafParamDict fields, List<LeafCurve> curves, LeafDeps deps) {
      ...
      linearPoints = CalcLinearPoints(curves.ConvertAll(c => new Curve3D(c)), deps).ToList();
    ...
    private void CreateVeins(LeafParamDict fields, LeafDeps deps) {
      ...
      bool hasLobes = fields[LPK.Heart].enabled;
    ...
    private void SplitVeins(LeafParamDict fields) {
      if (!fields[LPK.VeinSplit].enabled) return;
    ...
    public static (Vector3 point, float perpendicular) PointAlongMargin(float linearPoint, List<Vector3> linearPoints) {
      ...
      float angle = CurveJoin.Angle(marginStartPoint, marginEndPoint);
    ...
    public static List<Vector3> CalcLinearPoints(List<Curve3D> curves, LeafDeps deps) {
      Vector3 lastPoint;
      float sum = 0f;
      int count = 0;
      ...
      foreach (Curve3D curve in curves) {
        float lastSum = sum;
        lastPoint = curve.p0;
        for (int i = 1; i <= deps.baseParams.RenderLineSteps; i++) {
          if (i == 0) Debug.Log(lastPoint + "  --   " + curve);
          cachedPoints.Add(lastPoint);
          Vector3 nextPoint = curve.GetPoint(i / (float)deps.baseParams.RenderLineSteps);
      ...
      cachedPoints.Add(curves.Last().p1);

      float targetIncrement = deps.baseParams.LinearPointsIncr;
