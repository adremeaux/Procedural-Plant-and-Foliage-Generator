...
namespace BionicWombat {
  ...
  public class IMTextureFactory {
    ...
    public static IMTextureVars GetTextureVars(
        List<LeafCurve> curves, LeafVeins veins, LeafParamDict fields, LeafDeps deps, string leafName, PlantCollection collection) {
    ...
    public async Task DrawTexture(TextureType type, int downsampleOverride = -1) {
      ...
      void L(int n) { if (_startLoggin) Debug.Log(type + ": " + n); }
      ...
      try {
        ...
        L(5);
        DataManager.WriteIMTexture(image, vars.leafName, type, vars.collection);
    ...
    private void ApplyPixelCommands(TextureType type, PlantCollection collection) {
      Texture2D rgb = DataManager.GetTexture(vars.leafName, type, collection);
      ...
      DataManager.WriteTexture(rgb, vars.leafName, type, vars.collection);
    }
    ...
    private static PixelFillCommand BlendPFCColors(PixelFillCommand pfc, Color[] basePixels, float imgSizeScaled) {
      ...
      Color newColor = UnityEngine.Color.black;
      Color baseColor = UnityEngine.Color.black;
      for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
          int newY = h - y - 1;
          newColor = pfc.colors[y * w + x];
          if (pfc.useAlpha && newColor.a < 1f) {
            baseColor = basePixels[newY * w + x];
            newColor = UnityEngine.Color.Lerp(baseColor, newColor, newColor.a);
          } else {
            // newColor = new Color(newColor.r * newColor.a,
            //                      newColor.g * newColor.a,
            //                      newColor.b * newColor.a);
          }
          tColors[newY * w + x] = newColor;
        }
      }
