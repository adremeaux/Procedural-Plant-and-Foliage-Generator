...
namespace mattatz.Triangulation2DSystem {
  ...
  public class Polygon2D {
    ...
    public static Polygon2D Contour(Vector2[] points, Vector2[] additionalPoints = null) {
      var n = points.Length;
      ...
      while (true) {
        ...
        while (from2 != start && to2 != from) {
          if (flag = Utils2D.Intersect(from.p, to.p, from2.p, to2.p).intersect) {
            break;
          }
          from2 = to2;
          to2 = to2.to;
      ...
      return new Polygon2D(result.ToArray(), additionalPoints);
    }
    ...
    static List<HalfEdge2D> SplitEdges(List<HalfEdge2D> edges) {
      ...
      while (true) {
        ...
        while (to2 != from.from) {
          if (Utils2D.Intersect(from.p, to.p, from2.p, to2.p).intersect) {
            intersections++;
            if (intersections >= 2) {
              break;
            }
    ...
    public Polygon2D(Vector2[] contour, Vector2[] additionalPoints = null) {
      List<Vector2> combined = contour.ToList();
      if (additionalPoints != null) combined.AddRange(additionalPoints.ToList());
      vertices = combined.Select(p => new Vertex2D(p)).ToList();
