...
namespace mattatz.Triangulation2DSystem {
  ...
  public class Triangulation2D {
    ...
    public void Triangulate(Polygon2D polygon, float angle = 20f, float threshold = 0.1f) {
      angle = Mathf.Min(angle, kAngleMax) * Mathf.Deg2Rad;

      PSLG = polygon;
      V = PSLG.Vertices.ToList();
      S = PSLG.Segments.ToList();
      _Triangulate(polygon.Vertices.Select(v => v.Coordinate).ToArray(), angle, threshold);
    }
    ...
    void _Triangulate(Vector2[] points, float angle, float threshold) {
      Vector2 min, max;
